...
class MaskedAutoencoderViT(nn.Module, EncoderMixin):
    ...
    def forward_feature(self, x):
        ...
        features = [x.clone(), dummy]

        # embed patches
        # [batch_size, 3, 224, 224]
        x = self.patch_embed(x)
        # [batch_size, 196, 768] 14 * 14
        ...
        cls_tokens = cls_token.expand(x.shape[0], -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)
        # [batch_size, 197, 768] 14 * 14

        for i, blk in enumerate(self.blocks):
            x = blk(x)  # 224 / 16 = 14
            # [batch_size, 197, 768]
            feature = x[:, 1:, :].transpose(1, 2).reshape(B, self.embed_dim, H // self.patch_size, W // self.patch_size)
            # [batch_size, 196, 14, 14]
            feature = self.multi_scale_adaptors[i](feature)
...
def mae_vit_pet(**kwargs):
    model = MaskedAutoencoderViT(
        patch_size=16, out_chans=(512, 320, 128, 64), embed_dim=768, depth=4, num_heads=12,
